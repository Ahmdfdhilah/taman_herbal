...
<div class="container mx-auto mt-8">
  <div class="max-w-md mx-auto bg-white rounded-md overflow-hidden shadow-md p-6">
    <h2 class="py-4 font-bold text-center">Create Tanaman</h2>
    <form action="{{ route('tanaman.store') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-4">
        ...
        <input type="text" id="istilah" name="istilah" class="form-input" placeholder="Istilah">
        @error('istilah')
        <small class="text-red-500">{{ $message }}</small>
        @enderror
      </div>
      <div class="mb-4">
        ...
        <textarea id="penjelasan" name="penjelasan" class="form-textarea" placeholder="Penjelasan"></textarea>
        @error('penjelasan')
        <small class="text-red-500">{{ $message }}</small>
        @enderror
      </div>
