      ...
      <table class="table w-full border border-gray-300  bg-[#fff]">
        <thead class="bg-gray-200">
          <tr>
            ...
            <th class="py-2 px-4 border">Step</th>
            <th class="py-2 px-4 border">Manfaat</th>
        ...
        <tbody>
          ...
          @forelse ($pengelolaan as $data)
            <tr>
              ...
              <td class="py-2 px-4 border">{{ $data->step }}</td>
              <td class="py-2 px-4 border">{{ $data->manfaat }}</td>
              <td class="py-2 px-4 border">
                <img src="/image/{{ $data->img }}" class="img-fluid" alt="" width="90">
              </td>
              <td class="py-2 px-4 border">
                <div class="flex">
                  <a href="{{ route('pengelolaan.edit', $data->id) }}" class="btn btn-warning px-2 py-1 rounded-md bg-yellow-500">Edit</a>
                  <form action="{{ route('pengelolaan.destroy', $data->id) }}" method="POST" class="ml-2">
                    @csrf
                    @method('DELETE')
          ...
          @empty
            <tr>
              <td colspan="6" class="py-2 px-4 border text-center">Tidak ada data pengelolaan</td>
            </tr>
          @endforelse
        </tbody>
      </table>
