    ...
    <div class="overflow-x-auto">
      <table class="table w-full border border-gray-300  bg-[#fff]">
        ...
        <tbody>
          ...
          $i=1
          @endphp
          @forelse ($dokumentasi as $data)
          <tr>
            <td class="py-2 px-4 border">{{ $i++ }}</td>
            <td class="py-2 px-4 border">{{ $data->name }}</td>
            <td class="py-2 px-4 border">{{ $data->desc }}</td>
            <td class="py-2 px-4 border">
              <img src="/image/{{ $data->img }}" class="img-fluid" alt="" width="90">
            </td>
            <td class="py-2 px-4 border">
              <div class="flex">
                <a href="{{ route('dokumentasi.edit', $data->id) }}" class="btn btn-warning px-2 py-1 rounded-md bg-yellow-500">Edit</a>
                <form action="{{ route('dokumentasi.destroy', $data->id) }}" method="POST" class="ml-2">
                  @csrf
                  @method('DELETE')
